...
public class Car {
    ...
    * 시동이 걸여있는 지 상태를 확인해야 한다. */
    private boolean isOn; // 시동의 꺼짐과 켜짐을 나타내는 상태
    private int speed; // 자동차의 속력을 나타낸다.
    ...
    public void stop() {
        if(isOn){
            if(this.speed>0){
                this.speed = 0;
                System.out.println("끼이이이이이읶 브레이크를 밟았습니다. 차를 멈춥니다.");
            }else{
                System.out.println("차는 이미 멈춰있습니다.");
            }

        }else{
            System.out.println("차의 시동이 걸려 있지 않습니다. 시동 걸어주세요!");
        }
    }
    ...
    public void turnOff() {
        if(isOn){
            if(speed>0){
                System.out.println("달리는 상태에서는 시동은 끌 수 없습니다. 차를 먼저 멈춰주세요");
            } else{
                this.isOn = false;
